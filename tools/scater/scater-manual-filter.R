...
option_list = list(
  ...
  make_option(
   ...
   make_option(
    ...
    default = 0,
    type = 'numeric',
    help = "The number of reads mapped to a feature above which it to be deemed as expressed."
  ),
    make_option(
...
totalCounts <- sce$total > opt$library_size
sce <- sce[,totalCounts ]

print(paste(
"After filtering out low library counts: ", ncol(sce), "cells and", nrow(sce), "features."))

#Filter out high MT counts

mt_counts <- sce$subsets_Mito_percent < opt$percent_counts_MT
sce <- sce[,mt_counts]
